...
def trainer():
    ...
    try:
        saver.restore(sess, "nummodel.ckpt")
    except:
    ...
    saver.save(sess, "nummodel.ckpt")
    print("Model saved as nummodel.ckpt.")
...
def predictor():
    saver = tf.train.Saver()
    try:
        saver.restore(sess, "controllers/HAB/nummodel.ckpt")
    except:
        print("Model does not exist.")
        sys.exit(0)
    else:
